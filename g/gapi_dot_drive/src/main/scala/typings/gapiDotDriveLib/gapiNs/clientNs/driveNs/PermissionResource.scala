...
object PermissionResource {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(additionalRoles = additionalRoles, authKey = authKey, deleted = deleted, domain = domain, emailAddress = emailAddress, etag = etag, expirationDate = expirationDate, id = id, kind = kind, name = name, photoLink = photoLink, role = role, selfLink = selfLink, teamDrivePermissionDetails = teamDrivePermissionDetails, `type` = `type`, value = value, withLink = withLink)
