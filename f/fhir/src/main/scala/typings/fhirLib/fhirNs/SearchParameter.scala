...
- fhirLib.fhirNs.Resource because Already inherited */ trait SearchParameter extends DomainResource {
  /**
...
object SearchParameter {
  ...
  def apply(
    ...
    chain: js.Array[java.lang.String] = null,
    ...
    val __obj = js.Dynamic.literal(base = base, code = code, description = description, name = name, status = status, `type` = `type`, url = url)
