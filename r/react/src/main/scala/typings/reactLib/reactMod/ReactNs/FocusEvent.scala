...
object FocusEvent {
  ...
  def apply[T](
    ...
    val __obj = js.Dynamic.literal(bubbles = bubbles, cancelable = cancelable, currentTarget = currentTarget, defaultPrevented = defaultPrevented, eventPhase = eventPhase, isDefaultPrevented = isDefaultPrevented, isPropagationStopped = isPropagationStopped, isTrusted = isTrusted, nativeEvent = nativeEvent, persist = persist, preventDefault = preventDefault, relatedTarget = relatedTarget, stopPropagation = stopPropagation, target = target, timeStamp = timeStamp, `type` = `type`)
