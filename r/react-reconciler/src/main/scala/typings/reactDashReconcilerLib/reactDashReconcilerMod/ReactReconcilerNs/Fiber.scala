...
object Fiber {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(childExpirationTime = childExpirationTime, effectTag = effectTag, elementType = elementType, expirationTime = expirationTime, index = index, memoizedProps = memoizedProps, memoizedState = memoizedState, mode = mode, pendingProps = pendingProps, stateNode = stateNode, tag = tag, `type` = `type`)
