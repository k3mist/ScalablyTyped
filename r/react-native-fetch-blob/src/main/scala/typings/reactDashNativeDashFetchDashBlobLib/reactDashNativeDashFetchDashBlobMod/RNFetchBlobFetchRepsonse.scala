...
object RNFetchBlobFetchRepsonse {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(arrayBuffer = arrayBuffer, blob = blob, bodyUsed = bodyUsed, headers = headers, json = json, ok = ok, rawResp = rawResp, resp = resp, rnfbResp = rnfbResp, rnfbRespInfo = rnfbRespInfo, status = status, text = text, `type` = `type`)
