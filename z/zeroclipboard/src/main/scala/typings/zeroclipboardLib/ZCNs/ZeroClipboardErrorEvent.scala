...
object ZeroClipboardErrorEvent {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(currentTarget = currentTarget, message = message, name = name, relatedTarget = relatedTarget, target = target, timeStamp = timeStamp, `type` = `type`)
