...
object ZeroClipboardCopyEvent {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(clipboardData = clipboardData, currentTarget = currentTarget, relatedTarget = relatedTarget, target = target, timeStamp = timeStamp, `type` = `type`)
