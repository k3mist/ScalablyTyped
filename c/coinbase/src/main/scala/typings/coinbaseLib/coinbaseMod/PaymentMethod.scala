...
object PaymentMethod {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(allow_buy = allow_buy, allow_sell = allow_sell, currency = currency, instant_buy = instant_buy, instant_sell = instant_sell, name = name, primary_buy = primary_buy, primary_sell = primary_sell, resource = resource, `type` = `type`)
