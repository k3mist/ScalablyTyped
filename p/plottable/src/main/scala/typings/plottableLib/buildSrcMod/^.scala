package typings
package plottableLib.buildSrcMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("plottable/build/src", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  val AxisOrientation: plottableLib.Anon_Bottom = js.native
  val Renderer: plottableLib.Anon_Canvas = js.native
  val TimeInterval: plottableLib.Anon_Day = js.native
  val XAlignment: plottableLib.Anon_Center = js.native
  val YAlignment: plottableLib.Anon_BottomCenter = js.native
  var version: java.lang.String = js.native
}

