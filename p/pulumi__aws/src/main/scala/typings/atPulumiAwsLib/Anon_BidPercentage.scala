...
object Anon_BidPercentage {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(instanceRole = instanceRole, instanceTypes = instanceTypes, maxVcpus = maxVcpus, minVcpus = minVcpus, securityGroupIds = securityGroupIds, subnets = subnets, `type` = `type`)
