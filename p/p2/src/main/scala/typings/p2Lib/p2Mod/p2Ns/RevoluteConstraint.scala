...
object RevoluteConstraint {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(angle = angle, bodyA = bodyA, bodyB = bodyB, collideConnected = collideConnected, disableMotor = disableMotor, enableMotor = enableMotor, equeations = equeations, getMotorSpeed = getMotorSpeed, lowerLimit = lowerLimit, lowerLimitEnabled = lowerLimitEnabled, motorEnabled = motorEnabled, motorIsEnabled = motorIsEnabled, motorMaxForce = motorMaxForce, motorSpeed = motorSpeed, setLimits = setLimits, setMotorSpeed = setMotorSpeed, setRelaxation = setRelaxation, setStiffness = setStiffness, `type` = `type`, update = update, upperLimit = upperLimit, upperLimitEnabled = upperLimitEnabled)
