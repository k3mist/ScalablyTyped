...
object LockConstraint {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(bodyA = bodyA, bodyB = bodyB, collideConnected = collideConnected, equeations = equeations, getMaxForce = getMaxForce, setMaxForce = setMaxForce, setRelaxation = setRelaxation, setStiffness = setStiffness, `type` = `type`, update = update)
