...
object GearConstraint {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(angle = angle, bodyA = bodyA, bodyB = bodyB, collideConnected = collideConnected, equeations = equeations, getMaxTorque = getMaxTorque, ratio = ratio, setMaxTorque = setMaxTorque, setRelaxation = setRelaxation, setStiffness = setStiffness, `type` = `type`, update = update)
