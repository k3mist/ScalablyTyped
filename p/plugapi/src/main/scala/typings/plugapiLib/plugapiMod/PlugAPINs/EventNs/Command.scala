...
object Command {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(args = args, command = command, from = from, havePermission = havePermission, id = id, isFrom = isFrom, mentions = mentions, message = message, muted = muted, raw = raw, respond = respond, respondTimeout = respondTimeout, `type` = `type`)
