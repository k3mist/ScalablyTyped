...
object Anon_IdIn {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(id = id, in = in, label = label, links = links, maximumLinks = maximumLinks, name = name, parentNode = parentNode, selected = selected, `type` = `type`)
