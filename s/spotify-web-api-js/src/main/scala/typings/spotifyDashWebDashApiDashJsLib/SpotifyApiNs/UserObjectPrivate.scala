...
object UserObjectPrivate {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(birthdate = birthdate, country = country, email = email, external_urls = external_urls, href = href, id = id, product = product, `type` = `type`, uri = uri)
