...
object BankAccount {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(account_holder_name = account_holder_name, account_holder_type = account_holder_type, bank_name = bank_name, country = country, currency = currency, fingerprint = fingerprint, id = id, last4 = last4, `object` = `object`, routing_number = routing_number, status = status)
