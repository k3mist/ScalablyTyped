...
object RegExp {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(compile = compile, dotAll = dotAll, exec = exec, flags = flags, global = global, ignoreCase = ignoreCase, lastIndex = lastIndex, `match` = `match`, multiline = multiline, replace = replace, search = search, source = source, split = split, sticky = sticky, test = test, unicode = unicode)
