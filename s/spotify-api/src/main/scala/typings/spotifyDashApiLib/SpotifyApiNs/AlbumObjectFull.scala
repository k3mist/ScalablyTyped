...
object AlbumObjectFull {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(album_type = album_type, artists = artists, copyrights = copyrights, external_ids = external_ids, external_urls = external_urls, genres = genres, href = href, id = id, images = images, name = name, popularity = popularity, release_date = release_date, release_date_precision = release_date_precision, tracks = tracks, `type` = `type`, uri = uri)
