...
object StripeCard {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(createToken = createToken, exp_month = exp_month, exp_year = exp_year, last4 = last4, `object` = `object`, validateCVC = validateCVC, validateCardNumber = validateCardNumber, validateExpiry = validateExpiry)
