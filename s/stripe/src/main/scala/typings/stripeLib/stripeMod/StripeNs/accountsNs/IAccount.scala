...
object IAccount {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(charges_enabled = charges_enabled, country = country, details_submitted = details_submitted, display_name = display_name, id = id, `object` = `object`, payouts_enabled = payouts_enabled, verification = verification)
