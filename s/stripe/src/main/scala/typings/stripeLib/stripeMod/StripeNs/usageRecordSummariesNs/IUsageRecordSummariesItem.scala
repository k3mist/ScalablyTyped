...
object IUsageRecordSummariesItem {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(id = id, invoice = invoice, livemode = livemode, `object` = `object`, period = period, subscription_item = subscription_item, total_usage = total_usage)
