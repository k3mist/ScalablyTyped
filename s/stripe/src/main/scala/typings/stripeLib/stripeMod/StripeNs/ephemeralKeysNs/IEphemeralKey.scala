...
object IEphemeralKey {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(associated_objects = associated_objects, created = created, expires = expires, id = id, livemode = livemode, `object` = `object`, secret = secret)
