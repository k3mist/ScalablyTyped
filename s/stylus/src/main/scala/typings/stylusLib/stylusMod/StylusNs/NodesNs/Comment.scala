...
object Comment {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(clone = clone, coerce = coerce, column = column, eval = eval, filename = filename, first = first, hash = hash, `inline` = `inline`, lineno = lineno, nodeName = nodeName, operate = operate, shouldCoerce = shouldCoerce, str = str, suppress = suppress, toBoolean = toBoolean, toExpression = toExpression, toJSON = toJSON)
