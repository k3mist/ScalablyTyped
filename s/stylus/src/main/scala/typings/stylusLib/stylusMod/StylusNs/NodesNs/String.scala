...
object String {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(clone = clone, coerce = coerce, column = column, eval = eval, filename = filename, first = first, hash = hash, lineno = lineno, nodeName = nodeName, operate = operate, prefixed = prefixed, quote = quote, shouldCoerce = shouldCoerce, string = string, toBoolean = toBoolean, toExpression = toExpression, toJSON = toJSON, `val` = `val`)
