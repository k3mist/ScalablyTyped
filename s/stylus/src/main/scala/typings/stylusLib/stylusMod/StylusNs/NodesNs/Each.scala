...
object Each {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(block = block, clone = clone, coerce = coerce, column = column, eval = eval, expr = expr, filename = filename, first = first, hash = hash, key = key, lineno = lineno, nodeName = nodeName, operate = operate, shouldCoerce = shouldCoerce, toBoolean = toBoolean, toExpression = toExpression, toJSON = toJSON, `val` = `val`)
