...
object User {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(anonymous = anonymous, avatar = avatar, companies = companies, created_at = created_at, custom_attributes = custom_attributes, id = id, location_data = location_data.asInstanceOf[js.Any], segments = segments, session_count = session_count, social_profiles = social_profiles, tags = tags, `type` = `type`.asInstanceOf[js.Any], unsubscribed_from_emails = unsubscribed_from_emails, updated_at = updated_at)
