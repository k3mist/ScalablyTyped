...
object ListRouteView {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addCollapsibleSection = addCollapsibleSection, addSection = addSection, destroyed = destroyed, getParams = getParams, getRouteID = getRouteID, getRouteType = getRouteType, on_destroy = on_destroy, refresh = refresh)
