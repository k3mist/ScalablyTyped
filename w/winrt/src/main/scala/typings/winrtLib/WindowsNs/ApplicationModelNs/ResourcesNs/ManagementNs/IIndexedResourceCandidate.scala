...
object IIndexedResourceCandidate {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(getQualifierValue = getQualifierValue, metadata = metadata, qualifiers = qualifiers, `type` = `type`, uri = uri, valueAsString = valueAsString)
