...
object IContactInstantMessageField {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(category = category, displayText = displayText, launchUri = launchUri, name = name, service = service, `type` = `type`, userName = userName, value = value)
