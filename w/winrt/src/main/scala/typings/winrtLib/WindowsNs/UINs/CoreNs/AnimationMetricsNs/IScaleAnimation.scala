...
object IScaleAnimation {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(control1 = control1, control2 = control2, delay = delay, duration = duration, finalScaleX = finalScaleX, finalScaleY = finalScaleY, initialScaleX = initialScaleX, initialScaleY = initialScaleY, normalizedOrigin = normalizedOrigin, `type` = `type`)
