...
object IHostName {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(canonicalName = canonicalName, displayName = displayName, iPInformation = iPInformation, isEqual = isEqual, rawName = rawName, `type` = `type`)
