...
object ojGanttTaskSettableProperties {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(baseline = baseline, borderRadius = borderRadius, end = end, label = label, labelPosition = labelPosition.asInstanceOf[js.Any], labelStyle = labelStyle, progress = progress, start = start, svgClassName = svgClassName, svgStyle = svgStyle, `type` = `type`.asInstanceOf[js.Any])
