...
object TableChangedEventArgs {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(address = address, changeType = changeType.asInstanceOf[js.Any], details = details, getRange = getRange, getRangeOrNullObject = getRangeOrNullObject, source = source.asInstanceOf[js.Any], tableId = tableId, `type` = `type`, worksheetId = worksheetId)
