...
object Alias {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(hasComment = hasComment, hasProps = hasProps, jsonLike = jsonLike, props = props, rawValue = rawValue, `type` = `type`, valueRangeContainsNewline = valueRangeContainsNewline)
