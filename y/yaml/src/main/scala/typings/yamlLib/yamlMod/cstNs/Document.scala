...
object Document {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(anchor = anchor, comment = comment, contents = contents, directives = directives, hasComment = hasComment, hasProps = hasProps, jsonLike = jsonLike, props = props, tag = tag, `type` = `type`, valueRangeContainsNewline = valueRangeContainsNewline)
