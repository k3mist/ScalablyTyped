...
trait SeqItem extends Node {
  ...
  @JSName("type")
  var type_SeqItem: yamlLib.yamlLibStrings.SEQ_ITEM
}
...
object SeqItem {
  ...
  def apply(
    ...
    `type`: yamlLib.yamlLibStrings.SEQ_ITEM,
    ...
    val __obj = js.Dynamic.literal(hasComment = hasComment, hasProps = hasProps, jsonLike = jsonLike, props = props, `type` = `type`, valueRangeContainsNewline = valueRangeContainsNewline)
