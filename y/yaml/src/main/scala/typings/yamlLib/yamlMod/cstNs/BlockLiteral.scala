...
object BlockLiteral {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(chomping = chomping.asInstanceOf[js.Any], hasComment = hasComment, hasProps = hasProps, header = header, jsonLike = jsonLike, props = props, `type` = `type`, valueRangeContainsNewline = valueRangeContainsNewline)
