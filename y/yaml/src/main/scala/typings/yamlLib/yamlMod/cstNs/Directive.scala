...
object Directive {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(anchor = anchor, hasComment = hasComment, hasProps = hasProps, jsonLike = jsonLike, name = name, parameters = parameters, props = props, tag = tag, `type` = `type`, valueRangeContainsNewline = valueRangeContainsNewline)
