...
object Scope {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(block = block.asInstanceOf[js.Any], childScopes = childScopes, functionExpressionScope = functionExpressionScope, isStrict = isStrict, references = references, set = set, through = through, `type` = `type`.asInstanceOf[js.Any], variableScope = variableScope, variables = variables)
