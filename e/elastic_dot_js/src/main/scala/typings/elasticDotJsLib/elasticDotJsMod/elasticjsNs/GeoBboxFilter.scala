...
object GeoBboxFilter {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(_type = _type, bottomRight = bottomRight, cache = cache, cacheKey = cacheKey, field = field, name = name, normalize = normalize, toJSON = toJSON, topLeft = topLeft, `type` = `type`)
