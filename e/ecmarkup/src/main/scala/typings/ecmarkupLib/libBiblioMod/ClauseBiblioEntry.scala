...
object ClauseBiblioEntry {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(aoid = aoid, id = id, number = number.asInstanceOf[js.Any], referencingIds = referencingIds, title = title, titleHTML = titleHTML, `type` = `type`)
