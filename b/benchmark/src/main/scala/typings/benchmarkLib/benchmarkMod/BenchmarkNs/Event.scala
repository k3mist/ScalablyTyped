...
object Event {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(aborted = aborted, cancelled = cancelled, currentTarget = currentTarget, result = result, target = target, timeStamp = timeStamp, `type` = `type`)
