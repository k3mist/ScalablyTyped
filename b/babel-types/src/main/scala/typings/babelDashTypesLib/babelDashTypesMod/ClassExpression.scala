...
object ClassExpression {
  ...
  def apply(
    ...
    `type`: babelDashTypesLib.babelDashTypesLibStrings.ClassExpression,
    ...
    val __obj = js.Dynamic.literal(body = body, end = end, id = id, loc = loc, start = start, superClass = superClass, `type` = `type`)
