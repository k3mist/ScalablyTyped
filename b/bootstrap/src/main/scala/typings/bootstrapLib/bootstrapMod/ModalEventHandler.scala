...
object ModalEventHandler {
  ...
  def apply[TElement](
    ...
    val __obj = js.Dynamic.literal(currentTarget = currentTarget, delegateTarget = delegateTarget.asInstanceOf[js.Any], isDefaultPrevented = isDefaultPrevented, isImmediatePropagationStopped = isImmediatePropagationStopped, isPropagationStopped = isPropagationStopped, preventDefault = preventDefault, stopImmediatePropagation = stopImmediatePropagation, stopPropagation = stopPropagation, target = target, timeStamp = timeStamp, `type` = `type`)
