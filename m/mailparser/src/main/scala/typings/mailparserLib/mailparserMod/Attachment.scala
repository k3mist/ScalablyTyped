...
object Attachment {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(checksum = checksum, content = content, contentDisposition = contentDisposition, contentType = contentType, headers = headers, related = related, size = size, `type` = `type`)
