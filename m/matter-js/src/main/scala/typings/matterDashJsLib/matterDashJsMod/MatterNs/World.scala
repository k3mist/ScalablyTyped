...
object World {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(bodies = bodies, bounds = bounds, composites = composites, constraints = constraints, gravity = gravity, id = id, isModified = isModified, label = label, parent = parent, `type` = `type`)
