...
object MapTouchEvent {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(defaultPrevented = defaultPrevented, lngLat = lngLat, lngLats = lngLats, originalEvent = originalEvent, point = point, points = points, preventDefault = preventDefault, target = target, `type` = `type`.asInstanceOf[js.Any])
