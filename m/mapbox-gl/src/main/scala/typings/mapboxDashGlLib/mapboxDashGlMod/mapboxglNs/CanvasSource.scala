...
object CanvasSource {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(canvas = canvas.asInstanceOf[js.Any], coordinates = coordinates, getCanvas = getCanvas, pause = pause, play = play, setCoordinates = setCoordinates, `type` = `type`)
