...
object MapMouseEvent {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(defaultPrevented = defaultPrevented, lngLat = lngLat, originalEvent = originalEvent, point = point, preventDefault = preventDefault, target = target, `type` = `type`.asInstanceOf[js.Any])
