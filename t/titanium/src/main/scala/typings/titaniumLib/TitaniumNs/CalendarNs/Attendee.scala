...
object Attendee {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addEventListener = addEventListener, apiName = apiName, applyProperties = applyProperties, bubbleParent = bubbleParent, email = email, fireEvent = fireEvent, getApiName = getApiName, getBubbleParent = getBubbleParent, getEmail = getEmail, getIsOrganizer = getIsOrganizer, getName = getName, getRole = getRole, getStatus = getStatus, getType = getType, isOrganizer = isOrganizer, name = name, removeEventListener = removeEventListener, role = role, setBubbleParent = setBubbleParent, setEmail = setEmail, setIsOrganizer = setIsOrganizer, setName = setName, setRole = setRole, setStatus = setStatus, setType = setType, status = status, `type` = `type`)
