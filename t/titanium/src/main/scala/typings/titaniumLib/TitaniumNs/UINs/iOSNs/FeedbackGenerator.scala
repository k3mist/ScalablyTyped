...
object FeedbackGenerator {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addEventListener = addEventListener, apiName = apiName, applyProperties = applyProperties, bubbleParent = bubbleParent, fireEvent = fireEvent, getApiName = getApiName, getBubbleParent = getBubbleParent, getType = getType, impactOccurred = impactOccurred, notificationOccurred = notificationOccurred, prepare = prepare, removeEventListener = removeEventListener, selectionChanged = selectionChanged, setBubbleParent = setBubbleParent, setType = setType, `type` = `type`)
