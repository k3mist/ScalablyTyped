...
object IRuleMetadata {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(description = description, options = options, optionsDescription = optionsDescription, ruleName = ruleName, `type` = `type`, typescriptOnly = typescriptOnly)
