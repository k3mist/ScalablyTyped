...
object Groupbuy {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(count = count, detail = detail, discount = discount, etime = etime, groupbuy_price = groupbuy_price, original_price = original_price, photos = photos, provider = provider, sold_num = sold_num, stime = stime, ticket_address = ticket_address, ticket_tel = ticket_tel, title = title, `type` = `type`, type_code = type_code, url = url)
