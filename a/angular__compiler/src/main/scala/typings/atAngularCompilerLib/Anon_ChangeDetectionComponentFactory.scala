...
object Anon_ChangeDetectionComponentFactory {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(entryComponents = entryComponents, guards = guards, hostAttributes = hostAttributes, hostListeners = hostListeners, hostProperties = hostProperties, inputs = inputs, isComponent = isComponent, isHost = isHost, outputs = outputs, providers = providers, queries = queries, `type` = `type`, viewProviders = viewProviders, viewQueries = viewQueries)
