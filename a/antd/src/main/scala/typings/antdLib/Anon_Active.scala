...
object Anon_Active {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(default = default, format = format, gapDegree = gapDegree, percent = percent, showInfo = showInfo, status = status, strokeColor = strokeColor, strokeLinecap = strokeLinecap, strokeWidth = strokeWidth, trailColor = trailColor, `type` = `type`, width = width)
