...
object AudioPlayerPlaybackFailedRequest {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(currentPlaybackState = currentPlaybackState, error = error, requestId = requestId, timestamp = timestamp, token = token, `type` = `type`.asInstanceOf[js.Any])
