...
object AudioPlayerPlaybackRequest {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(offsetInMilliseconds = offsetInMilliseconds, requestId = requestId, timestamp = timestamp, token = token, `type` = `type`.asInstanceOf[js.Any])
