...
object VariableGroup {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(createdBy = createdBy, createdOn = createdOn, description = description, id = id, modifiedBy = modifiedBy, modifiedOn = modifiedOn, name = name, providerData = providerData, `type` = `type`, variables = variables)
