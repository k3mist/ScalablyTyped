...
object XamlBuildDefinition {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(_links = _links, batchSize = batchSize, buildArgs = buildArgs, continuousIntegrationQuietPeriod = continuousIntegrationQuietPeriod, controller = controller, createdDate = createdDate, createdOn = createdOn, defaultDropLocation = defaultDropLocation, description = description, id = id, lastBuild = lastBuild, name = name, path = path, project = project, queueStatus = queueStatus, repository = repository, revision = revision, supportedReasons = supportedReasons, triggerType = triggerType, `type` = `type`, uri = uri, url = url)
