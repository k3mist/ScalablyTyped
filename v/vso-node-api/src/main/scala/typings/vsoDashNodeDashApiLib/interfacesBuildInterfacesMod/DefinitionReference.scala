...
object DefinitionReference {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(createdDate = createdDate, id = id, name = name, path = path, project = project, queueStatus = queueStatus, revision = revision, `type` = `type`, uri = uri, url = url)
