...
object TimelineRecord {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(changeId = changeId, currentOperation = currentOperation, details = details, errorCount = errorCount, finishTime = finishTime, id = id, issues = issues, lastModified = lastModified, location = location, log = log, name = name, order = order, parentId = parentId, percentComplete = percentComplete, result = result, resultCode = resultCode, startTime = startTime, state = state, task = task, `type` = `type`, warningCount = warningCount, workerName = workerName)
