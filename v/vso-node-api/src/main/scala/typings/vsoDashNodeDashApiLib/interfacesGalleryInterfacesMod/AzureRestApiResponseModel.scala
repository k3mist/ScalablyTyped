...
object AzureRestApiResponseModel {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(assetDetails = assetDetails, assetId = assetId, assetVersion = assetVersion, customerSupportEmail = customerSupportEmail, integrationContactEmail = integrationContactEmail, operation = operation, operationStatus = operationStatus, planId = planId, publisherId = publisherId, `type` = `type`)
