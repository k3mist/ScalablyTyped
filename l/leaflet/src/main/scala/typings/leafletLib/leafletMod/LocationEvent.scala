...
object LocationEvent {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(accuracy = accuracy, altitude = altitude, altitudeAccuracy = altitudeAccuracy, bounds = bounds, heading = heading, latlng = latlng, speed = speed, target = target, timestamp = timestamp, `type` = `type`)
