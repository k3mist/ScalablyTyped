...
object MonthDay {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(adjustInto = adjustInto, atYear = atYear, compareTo = compareTo, dayOfMonth = dayOfMonth, equals = equals, format = format, get = get, getLong = getLong, isAfter = isAfter, isBefore = isBefore, isSupported = isSupported, isValidYear = isValidYear, month = month, monthValue = monthValue, query = query, range = range, toString = toString, `with` = `with`, withDayOfMonth = withDayOfMonth, withMonth = withMonth)
