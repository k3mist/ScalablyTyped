...
object NestedResults {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addResult = addResult, description = description, failedCount = failedCount, getItems = getItems, log = log, passed = passed, passedCount = passedCount, rollupCounts = rollupCounts, skipped = skipped, totalCount = totalCount, `type` = `type`)
