...
object PjaxPopStateEventObject {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(direction = direction, isDefaultPrevented = isDefaultPrevented, isImmediatePropagationStopped = isImmediatePropagationStopped, isPropagationStopped = isPropagationStopped, preventDefault = preventDefault, stopImmediatePropagation = stopImmediatePropagation, stopPropagation = stopPropagation, timeStamp = timeStamp, `type` = `type`)
